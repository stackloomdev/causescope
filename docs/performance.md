...
The repository deliberately accepts a frozen live-lab dependency that trails the repository manifest by at most one beta. A release PR must merge before that version can exist on npm, so an exact same-version lock cannot pass its pre-publish checks. After publishing, maintainers update the lab package and lockfile; CI rejects a second release while the previous sync is still missing.
